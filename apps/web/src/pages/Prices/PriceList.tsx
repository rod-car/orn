...
import { formatPrice, number_array, range } from "functions";
                        ...
                        <tr>
                            <td>
                                <Select
                                    controlled
                                    onChange={({ target }) =>
                                        updateQueryParam(target.value, "site_id")
                                    }
                            ...
                            <td>
                                <Select
                                    controlled
                                    onChange={({ target }) =>
                                        updateQueryParam(target.value, "article_id")
                                    }
                                    value={queryParams.article_id}
                                    options={articles}
                            ...
                            <td>
                                <Select
                                    controlled
                                    onChange={({ target }) =>
                                        updateQueryParam(target.value, "unit_id")
                                    }
                                    value={queryParams.unit_id}
                                    options={units}
                                    config={{ optionKey: "id", valueKey: "name" }}
                                    placeholder="Tous les unités"
                                />
                            </td>
                            <td>
                                <Select
                                    controlled
                                    onChange={({ target }) =>
                                        updateQueryParam(target.value, "perPage")
                                    }
                                    value={queryParams.perPage}
                                    options={number_array(100, 20)}
            ...
            <Block>
                <table className="table table-striped mb-4">
                    ...
                    <tbody>
                        ...
                        {!RequestState.loading && articlePrices?.total === 0 && <tr><td className="text-center" colSpan={5}>Aucune données</td></tr>}
                        {articlePrices &&
                            articlePrices.data?.map((articlePrice: ArticlePrice) => (
                                <tr key={articlePrice.id}>
                ...
                {articlePrices?.total > articlePrices?.per_page && (
                    <Pagination changePage={changePage} data={articlePrices} />
                )}
            </Block>
        </>
    );
